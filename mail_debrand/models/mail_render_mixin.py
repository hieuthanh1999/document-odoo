    ...
    def remove_href_odoo(
        ...
        if has_odoo_link:
            ...
            if to_keep:
                ...
                # Fix the style broken on email
                content_fix = '<div>fix_style_broken</div>'
                # END Fix the style broken on email
            else:
                to_change = [value]
                to_keep = ""
            new_parts = []
            # Fix the style broken on email
            length_keep_up = len(to_change) - 1
            previous_part = None
            # END Fix the style broken on email
            for part in to_change:
                ...
                # Fix the style broken on email
                if length_keep_up > 0:
                    add_to_keep = str(to_keep) + content_fix
                    part = part.replace(content_fix, add_to_keep)
                if previous_part is None:
                    previous_part = part
                else:
                    previous_part = previous_part.replace(content_fix, part)
                length_keep_up -= 1
                # END Fix the style broken on email
